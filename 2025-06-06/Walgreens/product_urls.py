...
def extract_product_urls(html):
    ...
    pdp_urls = [url for url in full_links if re.search(r'ID=prod\d+-product$', url)]
    ...
    for pid in product_ids:
        ...
        if product_urls:
            for product_url in product_urls:
                collection.insert_one({"product_id": pid, "product_url": product_url})
        else:
            collection.insert_one({"product_id": pid, "product_url": ""})

        print(f"{pid} URLs saved to DB")
        time.sleep(2)
