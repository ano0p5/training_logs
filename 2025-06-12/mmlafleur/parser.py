    ...
    def start(self):
        ...
        for record in urls:
            ...
            try:
                ...
                logging.info(f"Fetched {url} with status code: {res.status_code}")
    ...
    def parse_item(self, url, response):
        ...
        while True:
            review_url = (
                ...
                f"?page={page}&perPage=50&sort=date,rating,badge,images"
            )

            try:
                res = requests.get(review_url, headers=HEADERS)
                if res.status_code != 200:
                    logging.warning(f"Review API failed for {rid} on page {page}")
                    break
                ...
                logging.warning(f"Review fetch error for {url} on page {page}: {e}")
                break
